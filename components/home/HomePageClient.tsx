...
export function HomePageClient({ featuredPost, posts, error }: HomePageClientProps) {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${themeStyles.page}`} style={accentVariables}>
      <div className="mx-auto flex max-w-4xl flex-col gap-20 px-6 pb-20 pt-12 sm:px-8 sm:pt-16">
        ...
        <section className="space-y-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <a className="text-sm font-medium transition-colors duration-200" style={{ color: accentColor }} href="#archive">
              View archive →
            </a>
          </div>
          {error && (
            <div className={`rounded-md border p-6 text-sm ${themeStyles.surface}`}>
              <p className={themeStyles.bodyText}>
                Something went wrong while loading posts. Please refresh to try again.
              </p>
              <p className={`mt-2 text-xs ${themeStyles.subtleText}`}>{error}</p>
            </div>
          )}
          <div className="space-y-6">
            {filteredPosts.map((post) => (
              <PostCard key={post.id} post={post} theme={theme} themeStyles={themeStyles} />
            ))}
        ...
        <footer className={`rounded-md border p-8 text-sm transition-colors duration-300 ${themeStyles.surface}`}>
          <p className={themeStyles.bodyText}>
            Want notes in your inbox? Join the monthly dispatch and get the behind-the-scenes experiments before they ship.
          </p>
